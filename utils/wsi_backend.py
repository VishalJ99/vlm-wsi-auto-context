# ABOUTME: Shared WSI reader helpers with explicit cucim/openslide/isyntax selection and region extraction.
# ABOUTME: Provides consistent backend loading for scripts that need level-0 patch reads.
...
VALID_WSI_READERS = ("auto", "openslide", "cucim", "isyntax")
...
def load_wsi(wsi_path: str, wsi_reader: str = "auto"):
    ...
    Reader policy:
      ...
      - openslide/cucim/isyntax: force that backend.
      - auto: prefer openslide for .ndpi; otherwise prefer cucim.
    """
...
def close_wsi(wsi: Any, backend: str) -> None:
    if backend in {"openslide", "isyntax"} and hasattr(wsi, "close"):
        try:
            wsi.close()
        except Exception:
            pass
